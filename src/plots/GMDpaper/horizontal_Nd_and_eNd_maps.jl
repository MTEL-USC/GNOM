...
function plot_horizontal_maps!(fig)
    ...
    for (i, Δz) in enumerate(depths_intervals)
        ...
        Label(topscene, bbox = axs[i,2].scene.px_area, panellabels[i+5], textsize=20, halign=:left, valign=:bottom, padding=(10,0,5,0), font=labelfont, color=:white)
        Label(topscene, bbox = axs[i,1].scene.px_area, "Nd", textsize=20, halign=:left, valign=:top, padding=(60,0,0,35), font=labelfont, color=:white)
        Label(topscene, bbox = axs[i,1].scene.px_area, "($(Δz[1])–$(Δz[2]))m", textsize=10, halign=:left, valign=:top, padding=(60,0,0,60), font=labelfont, color=:white)
        Label(topscene, bbox = axs[i,2].scene.px_area, "εNd", textsize=20, halign=:left, valign=:top, padding=(60,0,0,35), font=labelfont, color=:white)
        Label(topscene, bbox = axs[i,2].scene.px_area, "($(Δz[1])–$(Δz[2]))m", textsize=10, halign=:left, valign=:top, padding=(60,0,0,60), font=labelfont, color=:white)
    end
    # colorbars
