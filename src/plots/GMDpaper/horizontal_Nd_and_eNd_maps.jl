...
function plot_horizontal_maps!(fig)
    ...
    for (i, Δz) in enumerate(depths_intervals)
        ...
        Makie.heatmap!(ax, sclons, lats, view(∫Nd, ilon, :), colormap=Ndcmap, nan_color=nan_color, colorrange=Ndclims)
        ...
        ax = axs[i,2]
        mapit!(ax, clon, mypolys(clon), color=continent_color)
        Makie.heatmap!(ax, sclons, lats, view(∫εNd, ilon, :), colormap=εcmap, nan_color=nan_color, colorrange=εclims)
    ...
    for (i, Δz) in enumerate(depths_intervals)
        text!(axs[i,1], panellabels[i], fontsize=20, align = (:left, :bottom), offset = (4, 2), space = :relative, font=labelfont, color=:white)
        text!(axs[i,2], panellabels[i+5], fontsize=20, align = (:left, :bottom), offset = (4, 2), space = :relative, font=labelfont, color=:white)
        text!(axs[i,1], 60, 50, text="Nd", fontsize=20, align = (:left, :bottom), offset = (4, -2), font=labelfont, color=:white)
        text!(axs[i,1], 60, 35, text="($(Δz[1])–$(Δz[2]))m", fontsize=10, align = (:left, :bottom), offset = (4, -2), font=labelfont, color=:white)
        text!(axs[i,2], 60, 50, text="εNd", fontsize=20, align = (:left, :bottom), offset = (4, -2), font=labelfont, color=:white)
        text!(axs[i,2], 60, 35, text="($(Δz[1])–$(Δz[2]))m", fontsize=10, align = (:left, :bottom), offset = (4, -2), font=labelfont, color=:white)
    end
    # colorbars
