...
function plot_profiles_v2!(fig)
    ...
    for (itracer, (xmodel, xobs, ux)) in enumerate(zip((DNdmodel, εNdmodel), (DNdobs, εNdobs), (uDNd, uεNd)))
        ...
        for (ibasin, isbasin) in enumerate(isbasins)
            ...
            Label(fig, bbox = ax.scene.px_area, reshape(panellabels[1:8], (4,2))[ibasin, itracer], fontsize=20, halign=:left, valign=:bottom, padding=(10,10,10,10), font=labelfont, color=:black)
